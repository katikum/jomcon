<?php
include ("project_alter.php");

class DeleteProject extends AlterProject {
    function __construct ($admin, $PID) {
        parent::__construct ("Remove Project", $admin, $PID);
    }

    function printBody () {
        ?>         
        <!-- Project Delete Start -->
        <div class="contact wow fadeInUp">
            <div class="container">
                <div class="section-header text-center">
                    <p>Are you sure?</p>
                    <h2>Delete Project</h2>
                </div>
                <div class="row">

                <?php parent::printProjectView ($this->data); ?>
                    
                    <div class="col-md-6">
                        <div class="contact-form">
                            <div id="success"></div>
                            <form name="sentMessage" id="contactForm" method="POST" action="project_delete.php">   
                                <input type="hidden" name = "PID" value = <?php echo ($this->data["ID"]); ?> />                           
                                <div>
                                    <h3>Are you sure you want to delete this project? This process cannot be reversed..</h3><hr>
                                </div>
                                <div>
                                    <button class="btn" type="submit" id="sendMessageButton" name ="delete" value=1>Go</button>
                                    <button class="btn" name ="delete" value=0>Cancel</button>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- Delete End -->
        <?php
    }
}

session_start ();

if (!isset ($_SESSION["admin"])) {
    header ("Location: index.php"); exit;
} else {
    if (isset ($_GET["id"])) {
        $pid = (int) $_GET["id"];
        if ($pid > 0) {  
            if (count (getRow ("projects", "*", ["ID", $pid])) > 0) {
                $del = new DeleteProject ($_SESSION["admin"], $pid);
                $del->printPage();
            } else { 
                header ("Location: my_projects.php"); exit;
            }
        } else {
            header ("Location: my_projects.php"); exit;
        }
    } else {
        if (isset ($_POST["delete"])) {            
            if ($_POST["delete"] == 1) {
                deleteRow ("projects", $_POST["PID"]);
            }
        }
        header ("Location: my_projects.php"); exit;
    }
}
?>
